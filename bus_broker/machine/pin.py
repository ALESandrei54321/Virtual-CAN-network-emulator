...
class Pin:
    ...
    def on(self):
        if Pin._states.get(self._id) != 1:
            Pin._states[self._id] = 1
            print(f"[Pin] GP{self._id:02d} → HIGH")

    def off(self):
        if Pin._states.get(self._id) != 0:
            Pin._states[self._id] = 0
            print(f"[Pin] GP{self._id:02d} → LOW")

    def toggle(self):
        Pin._states[self._id] = Pin._states.get(self._id, 0) ^ 1
    ...
    def value(self, v: int | None = None) -> int | None:
        ...
        new_val = int(bool(v))
        if Pin._states.get(self._id) != new_val:
            Pin._states[self._id] = new_val
            print(f"[Pin] GP{self._id:02d} → {'HIGH' if new_val else 'LOW'}")
        return None
